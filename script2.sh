...
# 4. Configuration spéciale pour le Config Service (overwrite)
cat > services/config-service.yml << 'EOF'
...
spring:
  ...
  cloud:
    config:
      server:
        git:
          uri: https://github.com/jireh325/SCD.git
          clone-on-start: true
          search-paths: 'SCD-config/{application}'  # Spécifiez le sous-dossier
          default-label: main  # Spécifiez la branche
